...
namespace GameLovers.MobileServices.Haptics.Internal
{
	...
	{
		/// <summary>True when the underlying device + OS can produce at least basic vibration; false otherwise.</summary>
		bool IsSupported { get; }

		/// <summary>Play the preset's natural one-shot duration.</summary>
		void PlayPresetOneShot(HapticPreset preset);

		/// <summary>Start looping the preset; loop continues until <see cref="Stop"/> or auto-stop coroutine fires.</summary>
		void PlayPresetLoop(HapticPreset preset);

		/// <summary>Play a single custom-intensity haptic. Intensity is in <c>[0, 1]</c>; duration in milliseconds.</summary>
