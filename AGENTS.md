...
- **`MobileServicesExplorerWindow`**: `Editor/Explorer/Windows/MobileServicesExplorerWindow.cs` + `.uss`. Menu item `Tools/GameLovers/Mobile Services Explorer`. `TabView` with 8 tabs and a top-row platform-skin dropdown (`iOS | Android`) that drives `MobileSimulatorState.WindowPlatform` (the standalone-window surface only — the runtime overlay carries its own `OverlayPlatform`). Header carries an inline `explorer-header-hint` label spelling that scope out so the next dev who pairs the Explorer with the Device Simulator plugin doesn't expect this dropdown to drive the in-Game-view overlay.
- **`MobileServiceTab` base** (`Editor/Explorer/Tabs/MobileServiceTab.cs`): abstract base mirroring the `ServiceTab` pattern in `com.gamelovers.services`. Implements the workspace UIToolkit gotcha-hardening pieces: play-mode-aware refresh scheduling, `tab-banner` empty-state, `MakePrimaryDangerButton`, `MakeStickyFoldout`, `TryShortCircuitRefresh`/`InvalidateRefreshDigest`. Tabs that nuke-and-rebuild their hierarchy MUST use `MakeStickyFoldout` + `TryShortCircuitRefresh` per the same workspace rules that apply to the services package.
- **Tabs** (`Editor/Explorer/Tabs/*.cs`): `OverviewTab`, `NativeUiTab`, `HapticsTab`, `NotificationsTab`, `GesturesTab`, `DeviceTab`, `PermissionsTab`, `AttDeepLinkTab`. Each is a `sealed` subclass of `MobileServiceTab`.
- **Truth-mirror simulator — two renderer surfaces, one broker** (`Editor/Explorer/Overlays/` + `Editor/Explorer/DeviceSimulatorPanel/`): both renderers subscribe to the same `MobileSimulatorState` (singleton broker / event bus) and reuse `MockBuilders` (per-shape factory methods) + the three USS files (`MobileSimulator.Common.uss`, `MobileSimulator.iOS.uss`, `MobileSimulator.Android.uss`) swapped at runtime when the platform flips. A persistent `[EDITOR SIMULATOR]` watermark is non-removable on every surface.
  - **Per-target routing**: every `Push*` carries a `SimulatorTarget` flag (default `All`); each renderer filters on its own bit (`StandaloneWindow` / `RuntimeOverlay`). The platform skin is also split per surface — `WindowPlatform` (Explorer dropdown) vs `OverlayPlatform` (Device Simulator plugin auto-sync) — so neither producer can clobber the other's surface. Producer→target mapping: Explorer tabs + `EditorPlatformSimulator` default to `All` (broadcast — they want every alive preview); the Device Simulator plugin scopes every push to `RuntimeOverlay` only.
  - **`MobileSimulatorWindow`** — dockable `EditorWindow`; menu item `Tools/GameLovers/Mobile Services Simulator Window`. Alive in edit + play mode. Subscribes to `WindowPlatformChanged` and filters every payload event on `SimulatorTarget.StandaloneWindow`. The original "open it next to the Game view" target.
  - **`MobileSimulatorRuntimeOverlay`** — editor-only `[InitializeOnLoad]` bootstrap that spawns a `[EditorOnly] MobileSimulatorOverlay` `DontDestroyOnLoad` GameObject with a UIDocument + programmatic `PanelSettings` (`sortingOrder = short.MaxValue`) on `EnteredPlayMode`. Tears down on `ExitingPlayMode` (clean teardown — no paused-snapshot mode). Opt-in via `MobileServicesSettings.EnableRuntimeSimulatorOverlay`. Subscribes to `OverlayPlatformChanged` and filters on `SimulatorTarget.RuntimeOverlay`. Renders pixel-aligned with the simulated device's `Screen.*` values so mocks land where Apple's reviewer would see them.
  - **`MobileServicesDeviceSimulatorPlugin`** (`Editor/Explorer/DeviceSimulatorPanel/`) — `UnityEditor.DeviceSimulation.DeviceSimulatorPlugin` subclass auto-discovered by Unity, embeds a slim Control Panel inside Unity's Device Simulator window. Auto-syncs `MobileSimulatorState.OverlayPlatform` from the selected device profile by reading `Application.platform` on a 500 ms `schedule.Execute` poll (robust across Unity 6 minor versions where `DeviceSimulator.deviceChanged` signature varies). Every action button passes `SimulatorTarget.RuntimeOverlay`, including its `Dismiss all mocks` button — the standalone Simulator window is never affected by anything fired here. The plugin's `Open full Explorer →` jump button is the seam back to the broadcast-by-default Explorer surface when needed. **Precondition banner**: an inline `msp-overlay-hint` amber banner near the top of the panel surfaces the runtime-overlay precondition (`MobileServicesSettings.EnableRuntimeSimulatorOverlay = true` AND play mode active), with a one-click `Open Project Settings →` jump. The same 500 ms poll that re-syncs the platform also refreshes the banner so it auto-hides the moment both preconditions are satisfied — without it, the panel's overlay-scoped routing would silently no-op in edit mode and the user would think the buttons were broken.
...
- **Runtime simulator overlay is Play-mode-only and Editor-asmdef-owned**
  - `MobileSimulatorRuntimeOverlay` lives in the Editor asmdef and spawns a `[EditorOnly]` `DontDestroyOnLoad` GameObject only when `MobileServicesSettings.EnableRuntimeSimulatorOverlay` is `true` and Unity enters play mode. The GameObject is destroyed instantly on `ExitingPlayMode`.
...
- Public API changes (`NativeUiService`, `INativeUiService`, `INotificationService` + `NotificationBuilder`, `IGameNotification`, `GestureController` events, `IHapticsService`, `IDeviceService` and any of its 8 child interfaces, `IDeepLinkRouter`, `IMobileService`)
...
- Editor surface changes (Explorer tabs, `EditorPlatformSimulator` API, internal introspection accessors, simulator USS / overlay payloads, menu items under `Tools/GameLovers/Mobile Services*`, `MobileSimulatorRuntimeOverlay` lifecycle, `MobileServicesDeviceSimulatorPlugin` panel layout / auto-platform-sync behaviour)
