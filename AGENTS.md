...
- **`MobileServicesExplorerWindow`**: `Editor/Explorer/Windows/MobileServicesExplorerWindow.cs` + `.uss`. Menu item `Tools/GameLovers/Mobile Services Explorer`. `TabView` with 8 tabs and a top-row platform-skin dropdown (`iOS | Android`) that pushes `MobileSimulatorState.Platform`.
