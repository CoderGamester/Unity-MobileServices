...
- **EditMode / Unit** (`EditMode/Unit/`): Pure-logic types and services whose Editor-platform code path is a log, override, or safe no-op. Includes:
  - `NativeUiService` (Editor log fallback plus alert show/dismiss overrides)
...
- **PlayMode / Unit** (`PlayMode/Unit/`): Anything that spawns or relies on `DontDestroyOnLoad` MonoBehaviours or Unity callback frames:
  ...
  - `GestureController` duplicate-input resilience (uses its internal press handler to reproduce a stale input index without reflection)
  - Use `[UnityTest]` returning `IEnumerator`.
- **PlayMode / Smoke** (`PlayMode/Smoke/`): Lightweight "instantiate without throwing" tests. `GestureController` lifecycle (enable/disable subscription) remains here; its duplicate-input resilience belongs in PlayMode Unit.
- **Editor tooling (NOT tested)**: types under `Editor/` are validated by manual editor smoke + on-device builds, not by automated tests. This explicitly includes `MobileServicesDeviceSimulatorPlugin`, `MobileSimulatorRuntimeOverlay`, `MobileSimulatorState`, `MockBuilders`, `EditorPlatformSimulator`, `MobileServicesConfig` / `MobileServicesConfigEditor` / `MobileServicesConfigMenuItems`, `MobileServicesScanner`, `MobileServicesBuildContext`, and `MobileServicesBuildPostprocessor`. The previous `GameLovers.MobileServices.Editor.Tests` assembly was removed in v1.0.0 — see §13 for rationale.
...
- A new disposable service test covers ownership, idempotency, and post-disposal behavior separately: its owned Unity object/registration is released, a second `Dispose()` is harmless, and every public operation that depends on the released resource throws `ObjectDisposedException`. Do not require that contract from stateless services merely because another service implements it.
- A component that acquires shared static/global state needs lifecycle coverage with two live owners. Disabling one must not break the other; cleanup must also tolerate the global facility being released externally before the component is disabled.
...
- Simulator/editor bridges that target a running service must unregister the exact target they registered. Tests or manual verification must prove the panel does not act on a parallel throwaway service instance.
...
| `PlayMode/Unit/` | `[UnityTest]` where a frame is required; `MobileNotificationService`, `DeviceServicesHost`, host-dependent services, callback receivers, `HapticsService` auto-stop, `GestureController` duplicate-input resilience |
...
|---|---|---|---|
| `NativeUiService` native paths (`Runtime/NativeUi/NativeUiService.cs:34`) | ACCEPTED | **(ii) engine-owned.** iOS `[DllImport]` and Android `AndroidJavaObject` behavior requires device smoke; Editor alert routing and validation are covered through the override hooks. | 2026-08-13 |
...
| `GestureController` duplicate input resilience (`Runtime/Gestures/GestureController.cs:128`) | CLOSED | `GestureControllerPlayModeTest.OnPressed_DuplicateFingerDown_DoesNotThrowAndReportsBothPresses` drives the internal press handler with a reused input index and proves that replacing stale tracking neither throws nor suppresses the second `Pressed` event. | 2026-08-08 |
| `GestureController` end-to-end gesture detection (`Runtime/Gestures/GestureController.cs:16`) | ACCEPTED | **(iii) harness-impossible** — blocker: driving `UnityEngine.InputSystem.EnhancedTouch.Touch` deterministically requires the Input System's `InputTestFixture`, which is not part of this package's test assembly. The lifecycle (subscribe/unsubscribe) is smoke-tested, duplicate-index resilience is covered above, and the math is fully covered through `ActiveGesture` / `SwipeInput` / `TapInput` unit tests. | 2026-08-08 |
...
| Process-wide editor-static leakage across fixtures (`Runtime/Device/**`, `Runtime/NativeUi/**`) | CLOSED | `NativeUiServiceTest` and `NativeUiServiceInstanceTest` restore alert show/dismiss overrides; review and battery fixtures retain their existing reset contracts. The `PermissionsService` / `AttService` half stays OPEN in the row above. | 2026-08-13 |
