...
| Build All reports a missing or duplicate scene catalog | Reimport **Mobile Services Samples** and keep exactly one `MobileServicesSampleBuildCatalog.asset`. The catalog stores serialized `SceneAsset` references, validates four unique pages in order, and derives current paths at runtime; it does not depend on hand-authored GUID/path strings or the currently open scene. |
